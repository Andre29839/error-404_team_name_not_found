import{L as d,r as b}from"./modal-footer-fbf715fa.js";const{BASIC_URL:v,API_KEY:u,trending_week:I,new_films:$,movie_detailes:M}=b,S="https://image.tmdb.org/t/p/original/",i={wrapper:document.querySelector(".month-wrapper"),addToLibraryBtn:document.querySelector('button[data-action="save"]'),container:document.querySelector(".container-img-weekly"),removeLibraryBtn:document.querySelector(".remove-from-library")};i.addToLibraryBtn.addEventListener("click",_);window.addEventListener("resize",k);i.removeLibraryBtn.addEventListener("click",B);JSON.parse(localStorage.getItem(d));let c;async function x(){const e=`${v}${$}?api_key=${u}`,t=await(await fetch(e)).json();if(t.results.length===0){const n=`<p class="error-text">OOPS...<br>
